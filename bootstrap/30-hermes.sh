...
if [[ -z "${HERMES_AGENT_RELEASE_TAG}" && "${HERMES_AGENT_VERSION}" == "0.11.0" ]]; then
  HERMES_AGENT_RELEASE_TAG="v2026.4.23"
fi
if [[ -n "${HERMES_AGENT_RELEASE_TAG}" ]] && ! [[ "${HERMES_AGENT_RELEASE_TAG}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+([.-][0-9A-Za-z]+)*$ ]]; then
  echo "ERROR: HERMES_AGENT_RELEASE_TAG must look like v2026.4.23." >&2
  exit 1
fi
...
if [[ "$current_version" != "$HERMES_AGENT_VERSION" ]]; then
  ...
  if [[ "$pypi_rc" -ne 0 ]]; then
    : "${HERMES_AGENT_RELEASE_TAG:?ERROR: PyPI install failed. Set HERMES_AGENT_RELEASE_TAG (example: v2026.4.23) in .env for GitHub tarball fallback.}"
    PIPX_HOME=/opt/pipx PIPX_BIN_DIR=/usr/local/bin \
